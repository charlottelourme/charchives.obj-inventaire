...
// L'overlay description est désormais en superposition INTERNE à la carte
// (1/3 bas, position:absolute dans .card-thumb-area) — plus besoin de
// calculer un débord vers la droite/gauche du viewport. Stub conservé
// pour la rétrocompatibilité d'appel et le resize listener.
function _positionPoemOverlays() { /* no-op : overlay interne à la carte */ }
...
function _renderNoteColorSwatches() {
  ...
  wrap.innerHTML = colors.map(c => {
    const isLight = c.hex === '#E5E5E5' || c.hex === '#F5E6D3' || c.hex === '#EDE8E0'
...
function _updateClusterLabels(labelsG, nodes) {
  ...
  const applyTypo = (sel) => sel
    ...
    .style('font-family', "'Compagnon', serif")
...
async function _dioRenderDecorBar(decBar, backdrop /*, _unused */) {
  ...
  for (let i = 0; i < DIO_SLOT_COUNT; i++) {
    ...
    if (slot && slot.blob) {
      ...
      // Supprime le slot (pour remplacer un fond : supprimer + ré-ajouter via le +).
...
function _renderTriosCards(objects) {
  ...
  if (_triosActiveTab === 'manuel') {
    ...
    grid.querySelectorAll('.trios-slot-lock').forEach(btn => {
      btn.addEventListener('click', e => {
        e.stopPropagation();
        const i = +btn.dataset.slot;
        _triosLockedSlots[i] = !_triosLockedSlots[i];
        const wrap = btn.closest('.trios-slot-wrap');
        const nowLocked = _triosLockedSlots[i];
        if (wrap) wrap.classList.toggle('locked', nowLocked);
        const title = nowLocked ? 'Libérer cet objet' : 'Fixer cet objet';
        btn.setAttribute('title', title);
        btn.setAttribute('aria-label', title);
        btn.setAttribute('aria-pressed', String(nowLocked));
        btn.innerHTML = _lockSvg(nowLocked);
        _renderTriosActions();
      });
    });
  }
...
// Comportement :
//   - Clic sur un mot → append à l'input (avec espace si nécessaire) + focus
//   - Bouton "Mélanger" → re-tire un nouvel échantillon de mots
function _renderOracleWordBox() {
  const box = document.getElementById('oracleWordBox');
  if (!box) return;
  const words = _sampleOracleWords();
  if (!words.length) {
    box.innerHTML = '';
    return;
  }
  const wordsHTML = words.map(w =>
    `<button class="oracle-wordbox-word" data-word="${esc(w)}" type="button">${esc(w)}</button>`
  ).join('');
  box.innerHTML = `
    <div class="oracle-wordbox-label">
      <span>Inspirations</span>
      <span class="oracle-wordbox-sep">·</span>
      <button class="oracle-wordbox-shuffle" id="oracleWordBoxShuffle" type="button">Mélanger</button>
    </div>
    <div class="oracle-wordbox-words">${wordsHTML}</div>
  `;
  // Clic mot → append à l'input
  box.querySelectorAll('.oracle-wordbox-word').forEach(btn => {
    btn.addEventListener('click', () => {
      const input = document.getElementById('oracleInput');
      if (!input) return;
      const w = btn.dataset.word;
      const current = (input.value || '').trim();
      input.value = current ? `${current} ${w}` : w;
      input.focus();
    });
  });
  // Mélanger → nouveau tirage
  document.getElementById('oracleWordBoxShuffle')?.addEventListener('click', _renderOracleWordBox);
}
...
const TRIOS_TAB_KEYS     = ['triosTab0', 'triosTab1'];
...
const BC_MAX_VISIBLE = 6;   // étapes visibles — si dépassement, préfixe "…"
